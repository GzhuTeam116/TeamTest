	...
	public void prepare(){
			...
			public void onClick(DialogInterface dialog, int id) {
				...
				case WARN_DIALOG_TYPE_WIFI:
					if(nm.openWIFIAdapter()){
						//Toast.makeText(getContext(), "WIFI已经开启", Toast.LENGTH_SHORT).show();
					}else{
		...
		setNegativeButton("取消", new OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int id) {
				// TODO Auto-generated method stub
				dialog.dismiss();
				Toast.makeText(getContext(), "已经取消操作", Toast.LENGTH_SHORT).show();
			}
		});
		...
		if(WarnDialogType == WARN_DIALOG_TYPE_WIFI){
			setMessage("WIFI连接已经关闭，是否现在开启？");
		}
		if(WarnDialogType == WARN_DIALOG_TYPE_BLUETOOTH){
			setMessage("蓝牙连接已经关闭，是否现在开启？");
		}
	}
}
